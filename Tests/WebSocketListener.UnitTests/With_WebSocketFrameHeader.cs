...
namespace WebSocketListenerTests
{
    ...
    public class With_WebSocketFrameHeader
    {
        ...
        public void With_WebSocketFrameHeaderFlags_Can_CreateMediumHeader_BiggerThanInt16()
        {
            ...
            var header = WebSocketFrameHeader.Create((Int64)ilength, true, false, WebSocketFrameOption.Text,
                new WebSocketExtensionFlags());
            Assert.AreEqual(4, header.HeaderLength);
            Byte[] buffer = new Byte[4];
            header.ToBytes(buffer, 0);
        ...
        {
            var header = WebSocketFrameHeader.Create(Int32.MaxValue, true, false, WebSocketFrameOption.Text,
                new WebSocketExtensionFlags());
        ...
        {
            var header = WebSocketFrameHeader.Create(Int64.MaxValue, true, false, WebSocketFrameOption.Text,
                new WebSocketExtensionFlags());
            Byte[] buffer = new Byte[10];
            header.ToBytes(buffer, 0);
            Assert.AreEqual(129, buffer[0]);
        ...
        {
            var header = WebSocketFrameHeader.Create(101, true, false, WebSocketFrameOption.Binary,
                new WebSocketExtensionFlags() { Rsv1 = true, Rsv2 = true });
            Byte[] buffer = new Byte[2];
            header.ToBytes(buffer, 0);
        ...
        public void With_WebSocketFrameHeaderFlags_Can_ParseSmallHeader()
        {
            ...
            Assert.IsTrue(WebSocketFrameHeader.TryParse(buffer, 0, 2, new ArraySegment<byte>(new Byte[4], 0, 4),
                out header));
        ...
        public void With_WebSocketFrameHeaderFlags_Can_ParseMediumHeader()
        {
            ...
            WebSocketFrameHeader header;
            Assert.IsTrue(WebSocketFrameHeader.TryParse(buffer, 0, 4, new ArraySegment<byte>(new Byte[4], 0, 4),
                out header));
            Assert.IsNotNull(header);
            Assert.IsTrue(header.Flags.FIN);
            Assert.IsFalse(header.Flags.MASK);
        ...
        public void With_WebSocketFrameHeaderFlags_Can_ParseBigHeader()
        {
            ...
            WebSocketFrameHeader header;
            Assert.IsTrue(WebSocketFrameHeader.TryParse(buffer, 0, 10, new ArraySegment<byte>(new Byte[4], 0, 4),
                out header));
            Assert.IsNotNull(header);
            Assert.IsTrue(header.Flags.FIN);
            Assert.IsFalse(header.Flags.MASK);
